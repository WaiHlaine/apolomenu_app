...
export const useCashierOrderItemStore = create<CashierOrderItemStore>()((set, get) => ({
    orders: {},
    sessions: {},
    menuItem: null,
    ...
    addOrUpdateOrderItem: (key, orderItem) =>
        set((state) => {
            ...
            const idx = existing.findIndex(
                (i) => i.menuItem?.id === orderItem.menuItem?.id && i.variantId === orderItem.variantId && i.notes.trim() === orderItem.notes.trim(),
            );
